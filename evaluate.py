...
from datasets.TID2013Dataset import TID2013Dataset, TID2013_GTtable
import numpy as np
import torch
...
def showfigures_TID13():
    def __find_test_img(gt_score):
        ...
        gt_table, _, _ = TID2013_GTtable('C:\\Users\\yunjeongyong\\Desktop\\DeepQA-yunjeong\\data\\all_data_csv\\TID2013.txt.csv')
    ...
    model = predictNet()
    model_dict = torch.load('C:\\Users\\yunjeongyong\\Desktop\\DeepQA-yunjeong\\data\\snapshot\\tid_0.8373_0.7944_epoch20.pth')['model']
    model.load_state_dict(model_dict)  # copyWeights(model, model_dict, freeze=False)
    model.eval()
    model.to('cuda')

    for batch_id, (img, ref, error, score_gt) in enumerate(testloader):
        ...
        score_gt = torch.tensor(score_gt, device='cuda', requires_grad=True, dtype=torch.float)
        ...
        plt.savefig('C:\\Users\\yunjeongyong\\Desktop\\DeepQA-yunjeong\\data\\results\\TID13_exp%d.png' % batch_id, dpi=500)
